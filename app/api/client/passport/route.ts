...
import { apiError, apiSuccess, noStore } from '@/lib/api-response'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return noStore(apiError('Pasaport oturumu hazırlanamadı', 503))
  }
  if (!auth) return noStore(apiError('Unauthorized', 401))
  ...
  if (!allowed) return noStore(apiError('Too many requests', 429))
  ...
  try {
    ...
    return noStore(apiSuccess(passport))
  } catch {
    console.error('[api/client/passport] GET failed')
    return noStore(apiError('Pasaport yüklenemedi', 500))
  }
}
