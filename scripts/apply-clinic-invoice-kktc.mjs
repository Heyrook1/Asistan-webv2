/**
 * Apply Q4 ClinicInvoice / KKTC e-Fatura migration (additive).
...
async function run() {
  ...
  const table = await client.query(`
    select to_regclass('public."ClinicInvoice"') as reg
  `)
  const enums = await client.query(`
    select t.typname
    from pg_type t
    where t.typname in ('ClinicInvoiceStatus', 'ClinicInvoiceKind')
    order by 1
  `)

  console.log('Business columns:', cols.rows.map((r) => r.column_name).join(', '))
  console.log('ClinicInvoice:', table.rows[0]?.reg)
  console.log('Enums:', enums.rows.map((r) => r.typname).join(', '))

  if (cols.rows.length !== 5) {
    throw new Error('Missing Business invoice columns after migration')
  }
  if (!table.rows[0]?.reg) {
    throw new Error('ClinicInvoice table missing after migration')
  }

  console.log('OK: clinic invoice migration applied')
}
